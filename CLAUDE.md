...
- **Supabase:** the schema is in `supabase/migrations/` (apply with `npx supabase db push`). Use `createClient()` from `src/lib/supabase/server.js` in server code (RLS applies), `src/lib/supabase/client.js` in Client Components and `createAdminClient()` from `src/lib/supabase/admin.js` only in `scripts/`. Competitor price history has no cascade, so never delete listings. **Auth:** Supabase email and password. Pages: `/login`, `/signup`, `/forgot-password` (guest pages, listed in `GUEST_PAGES` in `src/lib/auth.js`) and `/reset-password` (signed in). Signup also asks for a username, stored in `user_metadata.username` (a display name, not unique; login stays by email) and editable on `/dashboard/settings`. They share `AuthShell` and `AuthForm` (`useActionState`) with the actions in `src/app/actions/auth.js`. Confirmation and reset emails land on `/auth/callback?next=…` (PKCE code exchange). Every post-auth redirect goes through `safeNext()` (same-site paths only). `src/proxy.js` refreshes the session and redirects signed-out visitors to `/login?next=<path>` and signed-in ones away from guest pages, but only for the routes in its `matcher`, so marketing pages never pay for an auth check. Add every new signed-in route to that matcher, and still call `getClaims()` in the page and `redirect('/login')` without claims, because the proxy check is only optimistic. **Header:** the nav is a native `popover` (`#site-menu`) opened by the menu button below 60rem and shown inline above it. Signed in, the middle holds Dashboard and Reports (`NavLink` marks the current one) and the right holds the username, which opens an anchored `popover` submenu (`#account-menu`: Settings, Sign out); in the phone menu that submenu shows inline. `MenuCloser` closes both after a click inside. Signed-in state and the username come from `sessionUser()` (read from the session cookie, falling back to the email's local part), with no network call.

`project files/` is git-ignored and local only. It holds the product brief (`app_info.md`), the competitors in scope (`markets_information.md`), the data research (`data_sources_research.md`), and the old scrapers from `d:\work\scrapers`, which `src/lib/scrapers/` replaces. Still to build: `.github/workflows/scrape.yml`.

**Catalog:** `/dashboard` is the one product list. It has status tiles and chips, sorts by urgency, shows 50 per page, and has a live debounced search on `?q=` (name, brand or SKU, via `ProductSearch`). Add product and Import CSV open native `<dialog>` modals (`src/components/ProductDialog.js`). A product with no `match_key` shows as matching, and `MatchPoller` refreshes the page until it lands. `/dashboard/products` redirects to `/dashboard` (`next.config.mjs`). Each product has a page, `/dashboard/products/<id>` (linked from the dashboard table), that leads with suggestions: a suggested price from `suggestPrice()` in `match.js` (at risk: match the cheapest confirmed price; opportunity: a cent under it) with an Apply button (`applyPrice`), and the possible matches to accept. Below that are the key numbers, the matched competitor listings (promo, regular price, stores) and the merchant's own price history, plus Edit (dialog) and Delete. It adds, edits and deletes products and imports a CSV price list (`src/app/actions/products.js`). Parsing and validation live in `src/lib/catalog.js` (en/bg headers, `;` or `,`, decimal comma, Windows-1251). The `import_products` SQL function finds existing products by SKU (unique per merchant), or by name, brand and size, and updates only what changed. Saving a product (add or edit) matches just that product in the background with `after()` (`runMatch(..., { productIds })`), against the listings already scraped, so the response never waits on Gemini. This is the one app-side use of `createAdminClient()`, scoped to an id that came back through RLS. CSV imports still wait for `npm run match`.
...
- **Env (free-tier defaults):** `GEMINI_MODEL` (`gemini-3.5-flash-lite`), `GEMINI_RPM` (10), `MATCH_PAIRS_PER_REQUEST` (60), `MATCH_MAX_REQUESTS` (unlimited), `MATCH_CANDIDATES_PER_CHAIN` (3), `MATCH_MIN_SCORE` (0.35), `MATCH_MIN_CONFIDENCE` (0.7), `MATCH_PRICE_TOLERANCE` (0.03), `MATCH_ACTIVE_DAYS` (7).
